...
| **Human-in-the-Loop (HITL)** | Unified tool policy system governs ALL tools (built-in, custom, and MCP) with **default-deny** enforcement — unknown tools always require approval. Per-tool approval, proactive toggles, and **scope** (`global` = all users, `user` = admin only). Approval notifications show structured details: **action** (human-readable), **item** (device/entity name), **location**, **reason** (from agent reasoning), and **source** (User Chat / Scheduled Task / Proactive). Voice conversations exclude tools that require approval (cannot pause for HITL). Standing orders let users save approval preferences (Always Allow/Ignore/Reject). |
