...
| Frontend | Next.js 16 | App Router, Material UI (MUI v7) with 7 color themes, TailwindCSS, screen sharing via getDisplayMedia, **Markdown rendering** via `react-markdown` + `remark-gfm`, **virtualized message list** via `@tanstack/react-virtual` (windowed rendering, dynamic measurement, overscan buffer), header account dropdown with quick Profile and Sign out actions. **Middleware proxy** limit set to `50 MB` (`proxyClientMaxBodySize`) for large file uploads. Build requires `--webpack` flag (Turbopack unsupported). |
