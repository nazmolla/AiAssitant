...
describe("SchedulerConfig", () => {
  test("renders batch scheduler without throwing", async () => {
    const { SchedulerConfig } = await import("@/components/scheduler-config");
    expect(() => render(<SchedulerConfig />)).not.toThrow();
  });

  test("shows batch scheduling heading", async () => {
    const { SchedulerConfig } = await import("@/components/scheduler-config");
    render(<SchedulerConfig />);
    await waitFor(() => {
      expect(screen.getByText("Batch Scheduling")).toBeInTheDocument();
    }, { timeout: 3000 });
  });

  test("shows batch scheduler links", async () => {
    const { SchedulerConfig } = await import("@/components/scheduler-config");
    render(<SchedulerConfig />);
    await waitFor(() => {
...
// Legacy console tests commented out — moved to SchedulerConsole tests
/*
describe("SchedulerConfig — Legacy Console Tests (moved to SchedulerConsole)", () => {
