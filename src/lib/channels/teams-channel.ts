...
export class TeamsChannel extends BaseCommunicationChannel {
  ...
  async send(request: ChannelSendRequest): Promise<void> {
    ...
    if (!webhookUrl) {
      throw new Error("Teams channel missing webhookUrl.");
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(`Teams send failed: ${response.status} ${errBody}`);
    }
  }
}

export class TeamsChannelBuilder implements CommunicationChannelBuilder {
  constructor(private readonly fetchFn: FetchFn = fetch) {}

  matches(channel: ChannelRecord): boolean {
    return channel.channel_type === "teams";
  }

  create(channel: ChannelRecord): CommunicationChannel {
    return new TeamsChannel(channel, this.fetchFn);
  }
}
