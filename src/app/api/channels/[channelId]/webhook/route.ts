...
export async function POST(
  ...
  if (!channel.webhook_secret) {
    addLog({ level: "warning", source: "api.channel.webhook", message: "Webhook rejected: channel has no secret configured.", metadata: JSON.stringify({ channelId: channel.id }) });
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  // Accept either:
  //  (a) x-webhook-secret header with the raw secret (standard integrations), or
  //  (b) ?t=<hmac-token>&w=<window> query params for Twilio TwiML callbacks,
  //      where the token was generated by this server and binds to a 2-minute window.
  const headerSecret = req.headers.get("x-webhook-secret");
  const qToken = req.nextUrl.searchParams.get("t");
  const qWindow = req.nextUrl.searchParams.get("w");

  let authenticated = false;
  try {
    if (headerSecret) {
      // Constant-time comparison for header-based auth
    ...
    } else if (qToken && qWindow) {
      // Verify HMAC time-window token (valid for current and previous 1-minute window)
      const windowNum = parseInt(qWindow, 10);
      const nowMin = Math.floor(Date.now() / 60_000);
      for (const w of [nowMin, nowMin - 1]) {
        if (Math.abs(w - windowNum) > 1) continue;
        const expected = createHmac("sha256", channel.webhook_secret)
          .update(`${channel.id}:${w}`)
          .digest("hex")
          .slice(0, 32);
        const expectedBuf = Buffer.from(expected, "utf-8");
        const actualBuf = Buffer.from(qToken, "utf-8");
  ...
  if (!authenticated) {
    addLog({ level: "warning", source: "api.channel.webhook", message: "Webhook rejected: invalid or missing secret.", metadata: JSON.stringify({ channelId: channel.id }) });
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }
  ...
  try {
    ...
    if (channel.channel_type === "phone") {
      const voice = typeof channelConfig.voiceName === "string" && channelConfig.voiceName.trim().length > 0
        ...
        : "alice";
      // Use an HMAC-signed time-window token instead of the raw secret in the URL.
      // Twilio calls this URL back; we verify the token server-side.
      const windowMin = Math.floor(Date.now() / 60_000); // 1-minute window
      const twToken = createHmac("sha256", channel.webhook_secret || "")
        .update(`${channel.id}:${windowMin}`)
        .digest("hex")
        .slice(0, 32);
      const actionUrl = `${req.nextUrl.origin}/api/channels/${channel.id}/webhook?t=${twToken}&w=${windowMin}`;
