...
export async function POST(req: NextRequest) {
  ...
  if (!batchType || !["proactive", "maintenance", "email", "job_scout"].includes(batchType)) {
    return NextResponse.json({ error: "batch_type must be one of proactive|maintenance|email|job_scout" }, { status: 400 });
  }
