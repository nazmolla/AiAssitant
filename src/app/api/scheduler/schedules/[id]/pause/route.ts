...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  updateSchedulerScheduleStatus(schedule.id, "paused");
  addLog({
    level: "warning",
    source: "api.scheduler.control",
    message: "Paused scheduler schedule.",
    metadata: JSON.stringify({ userId: auth.user.id, scheduleId: schedule.id }),
  });

  return NextResponse.json({ ok: true, status: "paused", scheduleId: schedule.id });
}
